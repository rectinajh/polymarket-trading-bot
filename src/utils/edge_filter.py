...
class EdgeFilter:
    """
    Centralized edge filtering.

    Tightened after 2026-08 live drawdown: short sports props + loose
    thresholds produced ~14% win rate and large MTM losses.
    """

    # Disciplined thresholds
    MIN_EDGE_REQUIREMENT = 0.08        # 8% minimum edge
    HIGH_CONFIDENCE_EDGE = 0.06        # 6% for high confidence
    MEDIUM_CONFIDENCE_EDGE = 0.10      # 10% for medium confidence
    LOW_CONFIDENCE_EDGE = 0.15         # 15% for low confidence

    MIN_CONFIDENCE_FOR_TRADE = 0.60    # 60% minimum confidence
    MAX_ACCEPTABLE_RISK = 0.4          # 40% max position risk

    MIN_VOLUME_FOR_HIGH_EDGE = 5000    # Require liquid markets
    MIN_SPREAD_QUALITY = 0.02          # Tighter spreads
